    ...
    async with AsyncSessionLocal() as session:
        ...
        if not trades:
            ...
            for t in trades:
                ...
                total_pnl += pnl
                print(f"[{t.opened_at.time()}] {t.symbol} | {t.direction} | Entry: {t.entry_price} | Exit: {t.exit_price} | P&L: Rs.{pnl:.2f} | Status: {t.status}")
                
        print(f"\nTotal Realized P&L today: Rs.{total_pnl:.2f}\n")
        ...
        if not open_pos:
            ...
            for p in open_pos:
                ...
                print(f"{p.symbol} | {p.direction} | Qty: {p.size_units} | Entry: {p.entry_price} | Unrealized P&L: Rs.{unrealized:.2f}")
