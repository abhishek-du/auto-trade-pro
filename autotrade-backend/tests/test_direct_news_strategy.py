...
class TestDayRangeGate:
    """2026-07-29: check_day_range_stability() wired in alongside the
    price/volume gate -- see AASTHA.NS incident in
    tests/test_entry_confirmation.py::TestDayRangeStability."""

    @pytest.mark.asyncio
    async def test_unstable_day_range_is_not_traded(self):
        with patch("engine.entry_confirmation.check_price_volume_confirmation",
                   return_value=(True, "confirmed: 1.0% day move")), \
             patch("engine.entry_confirmation.check_day_range_stability",
                   return_value=(False, "day range too wide (23.4%) — elevated whipsaw/gap risk")), \
             patch("engine.decision_router.execute_trade_intent") as mock_execute:
            opened = await maybe_direct_trade("FOO.NS", "BUY", 123, _evidence(), "FOO rises on results")
        assert opened is False
        mock_execute.assert_not_called()
    ...
    @pytest.mark.asyncio
    async def test_day_range_check_runs_after_price_volume_gate_not_instead_of(self):
        """A signal that fails the PRIMARY (price/volume) gate should never
        reach the day-range check at all -- confirms gate ordering, not just
        that both exist independently."""
        with patch("engine.entry_confirmation.check_price_volume_confirmation",
                   return_value=(False, "price only +0.10% on the day")), \
             patch("engine.entry_confirmation.check_day_range_stability") as mock_range_check:
            opened = await maybe_direct_trade("FOO.NS", "BUY", 123, _evidence(), "FOO rises on results")
        assert opened is False
        mock_range_check.assert_not_called()

    @pytest.mark.asyncio
    async def test_stable_range_proceeds_to_execution(self):
        exec_result = SimpleNamespace(outcome=__import__(
            "engine.decision_router", fromlist=["RoutingOutcome"]
        ).RoutingOutcome.EXECUTED_PAPER, reason="ok")
        with patch("engine.entry_confirmation.check_price_volume_confirmation",
                   return_value=(True, "confirmed: 1.0% day move")), \
             patch("engine.entry_confirmation.check_day_range_stability",
                   return_value=(True, "day range stable (3.2%)")), \
             patch("news_discovery_engine._compute_news_trade_levels",
                   AsyncMock(return_value={"stop_loss": 95.0, "target_1": 110.0,
                                            "target_2": 115.0, "atr": 2.0})), \
             patch("paper_trading.virtual_wallet.VirtualWallet.get_summary",
